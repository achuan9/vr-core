...
export default {
  ...
  isOffScreen(obj, camera) {
    const frustum = new THREE.Frustum(); // Frustum用来确定相机的可视区域
   ...
   * @param {any} optEye 如果是vr模式，传入'left'或者'right'
   * @returns x坐标和y坐标
   */
  toScreenPosition(obj, camera, container, optEye) {
    ...
    if (optEye && size.width < size.height) {
      if (optEye == 'left' && target.y > size.height / 2) {
        target.y = -10000;
      }

      if (optEye == 'right' && target.y < size.height / 2) {
        target.y = -10000;
      }
    } else if (optEye && size.width >= size.height) {
      if (optEye == 'left' && target.x > size.width / 2) {
        target.x = -10000;
      }

      if (optEye == 'right' && target.x < size.width / 2) {
        target.x = -10000;
      }
    }
  ...
  */
   ...
   */
  lglt2xyz(lg, lt, r) {
    const x = r * Math.sin(lt) * Math.cos(lg);
    const y = r * Math.cos(lt);
    const z = r * Math.sin(lg) * Math.sin(lt);
  ...
  getVertexUvs(index, widthScale, heightScale) {
    ...
    const vectors = [
      [
        (((index - 1) % widthScale) + 1) / widthScale,
        1 - (parseInt((index - 1) / widthScale) % heightScale) / heightScale
      ],
      [
        ((index - 1) % widthScale) / widthScale,
        1 - (parseInt((index - 1) / widthScale) % heightScale) / heightScale
      ],
      [
        ((index - 1) % widthScale) / widthScale,
        1 - ((parseInt((index - 1) / widthScale) % heightScale) + 1) / heightScale
      ],
      [
        (((index - 1) % widthScale) + 1) / widthScale,
        1 - ((parseInt((index - 1) / widthScale) % heightScale) + 1) / heightScale
   ...
   * @returns
   */
  isPointInPolygon(poi, polylgon) {
    const p = turf.point([poi.x, poi.y]);
    const path = polylgon.map((dot) => [dot.x, dot.y]);
    path.push([polylgon[0].x, polylgon[0].y]);
    const poly = turf.polygon([path]);
    return turf.booleanPointInPolygon(p, poly);
  },
  /**
   * @description 加载图片
