 ...
 * @class MouseControl
 */
import { ANGLEFAR_MIN, ANGLNEAR_MAX, ANGLINIT_VALUE } from './constants';
export default class MouseControl {
  constructor(viewer) {
    ...
    this.viewer = viewer;
    this.fov = ANGLINIT_VALUE;
    this.maxFov = ANGLNEAR_MAX;
    this.minFov = ANGLEFAR_MIN;
    this.active = true;
  ...
  handleMouseWheel(e) {
    ...
    if (e.wheelDelta > 0) {
      // 向下滚，放大
      this.fov = Math.max(this.minFov, this.fov - 1);
    } else {
      // 向上滚，缩小
      this.fov = Math.min(this.maxFov, this.fov + 1);
    }
    this.viewer.setScale();
  }

  setMouseArg({ angleFar, angleNear, angleInit }) {
    this.minFov = angleFar || 0.5;
    this.maxFov = angleNear || 179;
    if (angleInit) {
      this.fov = angleInit;
      this.viewer.setScale();
    }
  ...
  getDistance(poi1, poi2) {
    return Math.sqrt(
      Math.pow(poi1.clientX - poi2.clientX, 2) + Math.pow(poi1.clientY - poi2.clientY, 2)
    );
  }
  ...
  handleMouseUp(e) {
    ...
    if (this.flag) {
      if (e.changedTouches && this.touchNum === 1) {
        ...
        function step() {
          const xvalue = easeOut(t, b, speedX, d);
          const yvalue = easeOut(t, b, speedY, d);
