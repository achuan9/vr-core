  ...
  updatePosition(camera) {
    ...
    if (this.traveller && this.traveller.isGlassesMode()) {
      ...
      if (utils.isOffScreen(this.tagMesh, camera)) {
        ...
        let rotateX = -(90 + THREE.MathUtils.radToDeg(camera.rotation.x));
        rotateX = Math.max(-60, Math.min(rotateX, -30));
        this.dom.style.transform = `translate3d(${position[0]}px, ${position[1]}px, 0) rotateZ(0deg) rotateX(${rotateX}deg) scale(1)`;
        // 'translate3d(' +
        // position[0] +
        // 'px, ' +
        // position[1] +
